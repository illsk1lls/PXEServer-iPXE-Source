...
static void dhcp_discovery_rx ( struct dhcp_session *dhcp,
    ...
    uint8_t option_data[16]; /* Buffer for option 200, big enough for "PXEServer" */
    size_t len;
    ...
    /* Check for option 200 with "PXEServer" */
    len = dhcppkt_fetch ( dhcppkt, 200, option_data, sizeof ( option_data ) );
    if ( len != strlen("PXEServer") || memcmp(option_data, "PXEServer", len) != 0 ) {
        DBGC ( dhcp, "DHCP %p rejecting packet: missing or invalid option 200 (expected 'PXEServer')\n", dhcp );
    ...
    /* If we don't have a DHCPOFFER, do nothing */
